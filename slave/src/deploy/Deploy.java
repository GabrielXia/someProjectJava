		...
		try {
			...
			while ((sCurrentLine = br.readLine()) != null) {
				...
				if(!b) { 
					...
					ProcessBuilder pbM = new ProcessBuilder("ssh", "jxia@"+ sCurrentLine, "mkdir -p /tmp/jxia/");
					pbM = pbM.inheritIO();
					Process pM = pbM.start();
					boolean bM = pM.waitFor(2, TimeUnit.SECONDS);
					
					ProcessBuilder pbC = new ProcessBuilder("scp", "-r", "/tmp/jxia/slave.jar","jxia@"+ sCurrentLine+":/tmp/jxia/slave.jar");
					pbC = pbC.inheritIO();
